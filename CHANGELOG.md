...
- Fixed a rapid disconnect/reconnect loop in all-port mode caused by recapturing Paqet's own raw tunnel packets.
- Corrected the Abroad owner-scoped TCP REDIRECT rule to bypass packets by tunnel **source** port instead of destination port.
- Added an Iran-side TPROXY bypass for TCP packets whose source port is the Paqet tunnel port.
- Added verification and integration coverage for both anti-loop rules and cleanup of the broken v4.1 rule.
- Applied stable SMUX buffers plus a 5-second keepalive and 30-second keepalive timeout on both roles, including preserved installations.
- Added Hev socket marking, longer transparent-proxy I/O timeouts, CPU-aware workers, systemd restart limits, and restart counters in `paqetx diagnose`.

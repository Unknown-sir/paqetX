...
readonly INSTALLER_VERSION="4.1.0"
...
cleanup_recorded_firewall_rules() {
  ...
  if [[ "$role" == "abroad" ]] && validate_port "$transport"; then
    ...
    if [[ "$service_uid" =~ ^[0-9]+$ ]]; then
      # Remove both the broken v4.0 DNAT rules and the corrected v4.1 REDIRECT rules.
...
apply_allport() {
  ...
  # Do not re-capture loopback traffic or sockets created by Hev itself.
...
apply_set() {
  ...
  if [[ "$role" == "abroad" ]]; then
    ...
    # address. Redirect only Paqet-owned destination sockets back to this Abroad
    # host while preserving the destination port. REDIRECT is more reliable than
    # DNAT-to-loopback across iptables legacy/nft backends.
    add_rule nat OUTPUT -m owner --uid-owner "$service_uid" -p tcp ! --dport "$transport" ! -d 127.0.0.0/8 -j REDIRECT
...
remove_set() {
  ...
  if [[ "$role" == "abroad" ]] && valid_port "$transport"; then
    ...
    if [[ "$service_uid" =~ ^[0-9]+$ ]]; then
      # Remove both v4.0 and v4.1 destination-rewrite variants.
...
verify_set() {
  ...
  if [[ "$role" == "abroad" ]]; then
    ...
    iptables -w 5 -t nat -C OUTPUT -m owner --uid-owner "$service_uid" -p tcp ! --dport "$transport" ! -d 127.0.0.0/8 -j REDIRECT >/dev/null 2>&1 || return 1
  elif [[ "$role" == "iran" && "$mode" == "all" ]]; then
...
Usage: sudo paqetx COMMAND
  ...
  diagnose   Show service, listener, routing, firewall, and packet counters

...
apply_allport() {
  ...
  if command -v modprobe >/dev/null 2>&1; then modprobe xt_TPROXY nf_tproxy_ipv4 >/dev/null 2>&1 || true; fi
  ...
  if [[ "$protocol_mode" == tcp || "$protocol_mode" == both ]]; then
    ...
    iptables -w 5 -t mangle -A "$ALLPORT_CHAIN" -p tcp -j TPROXY --on-port "$tproxy_port" --tproxy-mark "$TPROXY_MARK"
  fi
  if [[ "$protocol_mode" == udp || "$protocol_mode" == both ]]; then
    add_exclusion_rules udp "$transport" "$ssh_ports" "$excluded_ports"
    iptables -w 5 -t mangle -A "$ALLPORT_CHAIN" -p udp -j TPROXY --on-port "$tproxy_port" --tproxy-mark "$TPROXY_MARK"
  fi
  add_rule mangle PREROUTING -i "$interface" -m addrtype --dst-type LOCAL -j "$ALLPORT_CHAIN"
...
apply_set() {
  ...
  if [[ "$role" == "abroad" ]]; then
    ...
    # Paqet-created destination sockets always terminate on the Abroad host itself.
    add_rule nat OUTPUT -m owner --uid-owner "$service_uid" -p tcp ! --sport "$transport" -j DNAT --to-destination 127.0.0.1
    add_rule nat OUTPUT -m owner --uid-owner "$service_uid" -p udp -j DNAT --to-destination 127.0.0.1
  elif [[ "$role" == "iran" && "$mode" == "selected" && -n "$forwards" ]]; then
...
case "$cmd" in
  ...
  test)
    ...
    if [[ "$(role)" == "iran" ]]; then
      ...
      if has_tproxy; then systemctl is-active --quiet "$TPROXY" && echo "All-port helper is active."; fi
    else
      systemctl is-active "$SERVICE" && echo "Abroad service is active."
    fi
    ;;
  key)
    need_root

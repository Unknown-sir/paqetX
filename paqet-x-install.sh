#!/usr/bin/env bash
# Paqet X — guided installer for an Abroad server or an Iran entry server.
...
readonly TOKEN_PATH="${CONFIG_DIR}/connection.txt"
...
Run interactively on each server:
  curl -fsSL https://raw.githubusercontent.com/Unknown-sir/paqetX/main/paqet-x-install.sh | sudo bash

Roles:
  abroad   Foreign/Kharej server; runs Paqet in server mode.
  iran     Iran entry server; runs Paqet in client mode with port forwarding.

Non-interactive examples:
  # Abroad server
  sudo bash paqet-x-install.sh --role abroad --port 9999 --public-host 203.0.113.10 \
    --forward 443:443/tcp --forward 8443:8443/tcp

  # Iran server using the connection string printed by the abroad server
  sudo bash paqet-x-install.sh --role iran --connection-string 'paqetx://...'

Options:
  --role ROLE                 abroad|iran (aliases: server|client|kharej|foreign)
  --port PORT                 Paqet transport port, 1024-65535; not 80/443
  --public-host HOST          Public IPv4/DNS name of the abroad server
  --foreign-host HOST         Abroad server IPv4/DNS name (manual Iran setup)
  --connection-string TOKEN   Import settings printed by the abroad server
  --forward SPEC              Repeatable; PORT, LOCAL:TARGET, or LOCAL:TARGET/proto
                              Examples: 443, 8443:443, 53:53/udp
  --listen-ip IPV4            Iran listener address (default: 0.0.0.0)
  --target-host HOST          Target reached from abroad server (default: 127.0.0.1)
  --socks5 ADDRESS            Optional SOCKS5 listener, e.g. 127.0.0.1:1080
  --key KEY                   Shared key; generated automatically on abroad role
  ...
  --kcp-mode MODE             normal|fast|fast2|fast3 (default: fast)
  --block CIPHER              KCP cipher (default: aes-128-gcm)
  --conn NUMBER               Transport connections, 1-256 (default: 1)
  --mtu NUMBER                KCP MTU, 50-1500 (default: 1350)
  ...
  --yes                       Accept detected/default values without prompts
  --help                      Show this help

Rerun behavior:
  A normal rerun preserves the existing role and configuration. Use
  --reconfigure to run the role wizard again and replace it after a backup.
USAGE
}
...
validate_socks5() {
  ...
  host="${value%:*}"
  port="${value##*:}"
  validate_ipv4 "$host" && validate_port "$port"
}
...
prompt_role() {
  local choice=""
  tty_available || die "Role is required in non-interactive mode: --role abroad or --role iran"
  cat > /dev/tty <<'MENU'

Select this machine's role / نقش این سرور را انتخاب کنید:
  1) Abroad / خارج   — Paqet server next to your VPN service
  2) Iran / ایران    — Entry server with public port forwarding
MENU
  while true; do
    printf 'Choice [1-2]: ' > /dev/tty
    IFS= read -r choice < /dev/tty
    case "$choice" in
      1) printf 'abroad'; return 0 ;;
      2) printf 'iran'; return 0 ;;
...
make_connection_string() {
  local version="$1" host="$2" port="$3" key="$4" mode="$5" block="$6" conn="$7" mtu="$8"
  local forwards_json token_json spec
  forwards_json='[]'
  for spec in "${FORWARD_SPECS[@]}"; do
    forwards_json="$(jq -cn --argjson current "$forwards_json" --arg item "$spec" '$current + [$item]')"
  done
  token_json="$(jq -cn \
    --argjson schema 1 \
    --arg version "$version" \
    ...
    --argjson conn "$conn" \
    --argjson mtu "$mtu" \
    --argjson forwards "$forwards_json" \
    '{schema:$schema,version:$version,host:$host,port:$port,key:$key,mode:$mode,block:$block,conn:$conn,mtu:$mtu,forwards:$forwards}')"
  printf 'paqetx://%s' "$(printf '%s' "$token_json" | base64url_encode)"
}

parse_connection_string() {
  local token="$1" payload json item
  [[ "$token" == paqetx://* ]] || die "Invalid connection string prefix; expected paqetx://"
  payload="${token#paqetx://}"
  [[ ${#payload} -le 4096 ]] || die "Connection string is unexpectedly long"
  json="$(base64url_decode "$payload")" || die "Connection string is not valid base64url"
  jq -e 'type == "object" and .schema == 1 and (.forwards|type == "array")' >/dev/null <<<"$json" || die "Unsupported or malformed connection string"

  TOKEN_VERSION="$(jq -er '.version' <<<"$json")"
  FOREIGN_HOST="$(jq -er '.host' <<<"$json")"
  TRANSPORT_PORT="$(jq -er '.port|tostring' <<<"$json")"
  PAQET_KEY="$(jq -er '.key' <<<"$json")"
  KCP_MODE="$(jq -er '.mode' <<<"$json")"
  KCP_BLOCK="$(jq -er '.block' <<<"$json")"
  TRANSPORT_CONN="$(jq -er '.conn|tostring' <<<"$json")"
  KCP_MTU="$(jq -er '.mtu|tostring' <<<"$json")"

  validate_release_tag "$TOKEN_VERSION" || die "Invalid version in connection string"
  validate_host "$FOREIGN_HOST" || die "Invalid host in connection string"
  validate_transport_port "$TRANSPORT_PORT"
  validate_key "$PAQET_KEY" || die "Invalid key in connection string"
  validate_kcp_mode "$KCP_MODE" || die "Invalid KCP mode in connection string"
  validate_block "$KCP_BLOCK" || die "Invalid cipher in connection string"
  [[ "$TRANSPORT_CONN" =~ ^[0-9]+$ ]] && ((TRANSPORT_CONN >= 1 && TRANSPORT_CONN <= 256)) || die "Invalid conn value in connection string"
  [[ "$KCP_MTU" =~ ^[0-9]+$ ]] && ((KCP_MTU >= 50 && KCP_MTU <= 1500)) || die "Invalid MTU in connection string"

  while IFS= read -r item; do
    [[ -n "$item" ]] && append_forward_values "$item"
  done < <(jq -er '.forwards[]' <<<"$json")
}
...
self_test() {
  ...
  append_forward_values '443:443/tcp,8443:8443/tcp'
  token="$(make_connection_string 'v1.0.0-alpha.20' '203.0.113.10' '9999' '0123456789abcdef0123456789abcdef' 'fast' 'aes-128-gcm' '1' '1350')"
  FORWARD_SPECS=()
  parse_connection_string "$token"
  [[ "$FOREIGN_HOST" == '203.0.113.10' && "$TRANSPORT_PORT" == '9999' && ${#FORWARD_SPECS[@]} -eq 2 ]]
  printf 'Paqet X self-test passed.\n'
}

ROLE="${PAQET_ROLE:-}"
TRANSPORT_PORT="${PAQET_PORT:-9999}"
PUBLIC_HOST="${PAQET_PUBLIC_HOST:-}"
FOREIGN_HOST="${PAQET_FOREIGN_HOST:-}"
CONNECTION_STRING="${PAQET_CONNECTION_STRING:-}"
LISTEN_IP="${PAQET_LISTEN_IP:-0.0.0.0}"
TARGET_HOST="${PAQET_TARGET_HOST:-127.0.0.1}"
SOCKS5_ADDRESS="${PAQET_SOCKS5:-}"
PAQET_KEY="${PAQET_KEY:-}"
VERSION_REQUEST="${PAQET_VERSION:-}"
PAQET_INTERFACE="${PAQET_INTERFACE:-}"
PAQET_LOCAL_IP="${PAQET_LOCAL_IP:-}"
PAQET_ROUTER_MAC="${PAQET_ROUTER_MAC:-}"
KCP_MODE="${PAQET_KCP_MODE:-fast}"
KCP_BLOCK="${PAQET_BLOCK:-aes-128-gcm}"
TRANSPORT_CONN="${PAQET_CONN:-1}"
KCP_MTU="${PAQET_MTU:-1350}"
...
while (($# > 0)); do
  case "$1" in
    --role) (($# >= 2)) || die "--role requires a value"; ROLE="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
    --port) (($# >= 2)) || die "--port requires a value"; TRANSPORT_PORT="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
    --public-host) (($# >= 2)) || die "--public-host requires a value"; PUBLIC_HOST="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
    --foreign-host) (($# >= 2)) || die "--foreign-host requires a value"; FOREIGN_HOST="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
    --connection-string) (($# >= 2)) || die "--connection-string requires a value"; CONNECTION_STRING="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
    ...
    --socks5) (($# >= 2)) || die "--socks5 requires a value"; SOCKS5_ADDRESS="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
    --key) (($# >= 2)) || die "--key requires a value"; PAQET_KEY="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
    --version) (($# >= 2)) || die "--version requires a value"; VERSION_REQUEST="$2"; shift 2 ;;
    ...
    --kcp-mode) (($# >= 2)) || die "--kcp-mode requires a value"; KCP_MODE="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
    --block) (($# >= 2)) || die "--block requires a value"; KCP_BLOCK="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
    --conn) (($# >= 2)) || die "--conn requires a value"; TRANSPORT_CONN="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
    --mtu) (($# >= 2)) || die "--mtu requires a value"; KCP_MTU="$2"; CONFIG_INPUT_GIVEN=1; shift 2 ;;
...
if [[ "$SELF_TEST" == "1" ]]; then
  command -v jq >/dev/null 2>&1 || die "jq is required for --self-test"
  command -v base64 >/dev/null 2>&1 || die "base64 is required for --self-test"
  self_test
  exit 0
fi
...
validate_transport_port "$TRANSPORT_PORT"
[[ -z "$PUBLIC_HOST" ]] || validate_host "$PUBLIC_HOST" || die "Invalid --public-host: ${PUBLIC_HOST}"
...
[[ -z "$SOCKS5_ADDRESS" ]] || validate_socks5 "$SOCKS5_ADDRESS" || die "Invalid SOCKS5 address; use IPv4:PORT"
[[ -z "$VERSION_REQUEST" ]] || validate_release_tag "$VERSION_REQUEST" || die "Invalid release tag: ${VERSION_REQUEST}"
...
install_dependencies() {
  local -a required=(curl jq tar ip iptables systemctl sha256sum install awk sed grep find base64 od)
  local -a missing=()
  local cmd
...
step "Loading existing configuration or selecting a role"
...
if [[ -f "$CONFIG_PATH" && "$RECONFIGURE" != "1" ]]; then
  ...
  # Ignore new configuration arguments and restore the previous installer state.
  FORWARD_SPECS=()
  SAVED_PUBLIC_HOST="$(read_state_value PUBLIC_HOST)"
  ...
  [[ -z "$SAVED_PUBLIC_HOST" ]] || PUBLIC_HOST="$SAVED_PUBLIC_HOST"
  ...
  [[ -n "$ROLE" ]] || ROLE="$(prompt_role)"
  ROLE="$(normalize_role "$ROLE")" || die "Invalid role. Use abroad or iran"

  if [[ "$ROLE" == "iran" && -n "$CONNECTION_STRING" ]]; then
    parse_connection_string "$CONNECTION_STRING"
    [[ -n "$VERSION_REQUEST" ]] || VERSION_REQUEST="$TOKEN_VERSION"
    log_success "Imported abroad host, key, version, and forwarding presets from connection string"
  fi

  if [[ "$ROLE" == "abroad" ]]; then
    if [[ "$ASSUME_YES" != "1" && -z "${PAQET_PORT:-}" ]]; then
      prompt_value TRANSPORT_PORT "Paqet transport port / پورت تونل" "$TRANSPORT_PORT"
      validate_transport_port "$TRANSPORT_PORT"
    fi
    if [[ -z "$PAQET_KEY" ]]; then
    ...
    if ((${#FORWARD_SPECS[@]} == 0)) && [[ "$ASSUME_YES" != "1" ]]; then
      FORWARD_INPUT=""
      prompt_value FORWARD_INPUT "Suggested Iran forwards (e.g. 443 or 8443:443, comma-separated)" ""
      [[ -z "$FORWARD_INPUT" ]] || append_forward_values "$FORWARD_INPUT"
    fi
  else
    if [[ -z "$CONNECTION_STRING" && -z "$FOREIGN_HOST" && "$ASSUME_YES" != "1" ]]; then
      prompt_value CONNECTION_STRING "Paste paqetx:// connection string (leave blank for manual setup)" ""
      [[ -z "$CONNECTION_STRING" ]] || { parse_connection_string "$CONNECTION_STRING"; [[ -n "$VERSION_REQUEST" ]] || VERSION_REQUEST="$TOKEN_VERSION"; }
    fi
    ...
    if [[ -z "$PAQET_KEY" ]]; then
      prompt_value PAQET_KEY "Shared key / کلید مشترک" "" 1
    fi
    validate_key "$PAQET_KEY" || die "Invalid shared key"
    if ((${#FORWARD_SPECS[@]} == 0)); then
      FORWARD_INPUT=""
      prompt_value FORWARD_INPUT "Iran forwards (e.g. 443 or 8443:443, comma-separated)" ""
      [[ -n "$FORWARD_INPUT" ]] || die "At least one port-forward mapping is required on the Iran role"
      append_forward_values "$FORWARD_INPUT"
    fi
  fi
fi
...
if [[ "$PRESERVE_CONFIG" != "1" ]]; then
  if [[ -f "$CONFIG_PATH" ]]; then
    ...
    [[ ! -f "$TOKEN_PATH" ]] || cp -a "$TOKEN_PATH" "${TOKEN_PATH}.bak.${BACKUP_SUFFIX}"
    log_warn "Previous configuration backed up with suffix ${BACKUP_SUFFIX}"
  fi

  if [[ "$ROLE" == "abroad" ]]; then
    if [[ -z "$PUBLIC_HOST" ]]; then
      PUBLIC_HOST="$(curl -4fsS --connect-timeout 4 --max-time 8 https://api.ipify.org 2>/dev/null || true)"
      validate_ipv4 "$PUBLIC_HOST" || PUBLIC_HOST="$PAQET_LOCAL_IP"
    fi
    if [[ "$ASSUME_YES" != "1" ]]; then
      DETECTED_PUBLIC="$PUBLIC_HOST"
      prompt_value PUBLIC_HOST "Abroad public IP or DNS / آدرس عمومی خارج" "$DETECTED_PUBLIC"
    fi
    validate_host "$PUBLIC_HOST" || die "Invalid public host: ${PUBLIC_HOST}"
...
log:
  level: "info"
CONFIG
      if [[ -n "$SOCKS5_ADDRESS" ]]; then
        cat <<CONFIG

socks5:
  - listen: "${SOCKS5_ADDRESS}"
    username: ""
    password: ""
CONFIG
      fi
...
CONFIG
  ...
  # Recover values needed by services, firewall state, and the connection token.
  ...
  if [[ "$ROLE" == "abroad" ]]; then
    ...
    validate_transport_port "$TRANSPORT_PORT"
    [[ -n "$PUBLIC_HOST" ]] || PUBLIC_HOST="$PAQET_LOCAL_IP"
    validate_host "$PUBLIC_HOST" || die "Invalid preserved public host"
  else
...
if [[ "$ROLE" == "abroad" ]]; then
  CONNECTION_STRING="$(make_connection_string "$RELEASE_TAG" "$PUBLIC_HOST" "$TRANSPORT_PORT" "$PAQET_KEY" "$KCP_MODE" "$KCP_BLOCK" "$TRANSPORT_CONN" "$KCP_MTU")"
  printf '%s\n' "$CONNECTION_STRING" > "$TOKEN_PATH"
...
readonly TOKEN="/etc/paqet-x/connection.txt"
...
Usage: sudo paqetx COMMAND
  ...
  stop       Stop services
  test       Run Paqet ping on Iran role; show status on abroad role
  token      Show the paqetx:// connection string on the abroad server
...
case "$cmd" in
  ...
  test)
    ...
    ;;
  token) need_root; [[ -r "$TOKEN" ]] || { echo "Connection string exists only on the abroad role." >&2; exit 1; }; cat "$TOKEN" ;;
...
if [[ "$SKIP_START" == "1" ]]; then
  ...
  if [[ "$ROLE" == "iran" ]]; then
    if "$BIN_PATH" ping -c "$CONFIG_PATH" >/dev/null 2>&1; then
      log_success "Iran-to-abroad Paqet ping succeeded"
    else
      log_warn "Service is running, but Paqet ping did not succeed yet. Check cloud firewall, host, key, and matching version"
    fi
  fi
fi
...
if [[ "$ROLE" == "abroad" ]]; then
  printf '  Public server: %s:%s\n' "$PUBLIC_HOST" "$TRANSPORT_PORT"
  printf '\n%sConnection string for the Iran server:%s\n%s\n' "$BOLD" "$RESET" "$CONNECTION_STRING"
  printf '\nRun the installer on the Iran server and paste this string.\n'
  printf 'Also allow inbound TCP %s in the cloud/provider firewall.\n' "$TRANSPORT_PORT"
else
  printf '  Abroad server: %s:%s\n' "$FOREIGN_HOST" "$TRANSPORT_PORT"
  printf '  Forwards:      %s\n' "$(forward_specs_csv)"
  printf '\nPoint users/apps to this Iran server and the configured listen ports.\n'
fi
printf '\nUseful commands:\n'

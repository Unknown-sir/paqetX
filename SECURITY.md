...
The Abroad role installs the required Paqet raw-TCP rules plus UID-scoped REDIRECT destination handling for same-port local services. Selected-port Iran mode opens only the recorded TCP/UDP ports. All-port Iran mode creates a dedicated mangle chain, TPROXY mark, and policy-routing table while explicitly excluding the tunnel destination port, detected SSH ports, user exclusions, and packets arriving from the tunnel source port. The Abroad REDIRECT rule also bypasses raw tunnel replies by source port to prevent recapture loops.
...
Run `sudo paqetx diagnose` on both hosts after installation. The command shows only system state and firewall counters; note that `sudo paqetx config` displays the shared secret and should not be posted publicly.

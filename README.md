...
- **All Ports:** فوروارد هم‌زمان تمام پورت‌های IPv4 ورودی در ایران با **TCP و UDP**
...
تمام پورت‌های ورودی IPv4 روی سرور ایران، همیشه برای **هر دو پروتکل TCP و UDP** و با همان شماره پورت به سرویس متناظر در سرور خارج منتقل می‌شوند، به‌جز:
...
قبل از اعمال Ruleها، استثناهای نهایی نمایش داده می‌شوند و نیاز به تأیید دارند. برای هر استثنا دو Rule مستقل TCP و UDP قبل از Ruleهای catch-all ساخته و سپس اعتبارسنجی می‌شود.

## ارتقا از نسخه 5.1.0

برای اصلاح Ruleهای قدیمی، نسخه جدید را روی سرور ایران دوباره اجرا و همان سرویس All Ports را بازسازی کنید. نصب‌کننده chainهای `PQX_*` باقی‌مانده را پاک می‌کند و یک chain ثابت `PQX_ALLPORT` می‌سازد. پس از نصب، در خروجی Ruleها باید برای هر پورت استثنا هم TCP و هم UDP قبل از TPROXY دیده شود.
...
- روی هر سرور ایران فقط یک سرویس All Ports فعال می‌شود، زیرا ورودی Local سیستم را در اختیار می‌گیرد. نسخه 5.2.0 هنگام نصب، chainهای باقی‌مانده از نسخه‌های قدیمی را پاک می‌کند تا Rule قدیمی قبل از استثناهای جدید اجرا نشود.
...
- All incoming IPv4 ports forwarding on Iran with TCP **and** UDP simultaneously
...
Captures incoming IPv4 traffic on the Iran server for **both TCP and UDP**, forwarding each connection/datagram to the same destination port on the Kharej server, excluding:
...
The installer detects SSH ports from the current session, effective sshd configuration, active sshd sockets and SSH configuration files. It shows the final exclusion list before applying rules. Every exclusion is installed for both TCP and UDP ahead of the catch-all rules, and stale Paqet-X chains from earlier releases are removed first.

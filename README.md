...
### رفع قطع‌و‌وصل سریع در نسخه 4.2.0

در نسخه 4.1، حالت همه‌پورت می‌توانست بسته‌های خود تونل را دوباره وارد مسیر Forward کند:

- روی سرور خارج، قانون `REDIRECT` به‌اشتباه `dport` را مستثنا می‌کرد؛ در حالی که پاسخ‌های خام Paqet پورت تونل را در `sport` دارند. نتیجه این بود که پاسخ تونل دوباره به سرویس محلی Redirect می‌شد.
- روی سرور ایران، پاسخ‌های تونل با `sport=پورت تونل` وارد chain مربوط به TPROXY می‌شدند و به Hev تحویل داده می‌شدند.

این دو مسیر یک حلقه بازگشتی می‌ساختند و باعث می‌شد اتصال هر چند ثانیه بسته و دوباره ساخته شود. نسخه 4.2 این مشکل را با دو قانون ضدحلقه رفع می‌کند:

- خارج فقط سوکت‌های مقصد Paqet را با شرط `! --sport <TUNNEL_PORT>` Redirect می‌کند.
- ایران تمام بسته‌های TCP با `--sport <TUNNEL_PORT>` را پیش از TPROXY مستثنا می‌کند.

همچنین پروفایل پایداری SMUX روی هر دو سمت اعمال می‌شود: bufferهای رسمی 4MB/2MB، keepalive پنج‌ثانیه‌ای و timeout سی‌ثانیه‌ای. systemd نیز دارای محدودیت restart است تا در صورت خطای واقعی وارد حلقه بی‌نهایت نشود. تنظیمات مقصد، Key، نقش و پورت‌ها در اجرای مجدد حفظ می‌شوند.

اصلاحات نسخه 4.1 شامل hook سراسری PREROUTING، policy routing، بررسی listenerها و دستور `sudo paqetx diagnose` نیز همچنان باقی هستند.

برای ارتقا از نسخه‌های 4.0 یا 4.1، اسکریپت جدید را **روی هر دو سرور** دوباره اجرا کنید. کانفیگ و Key فعلی حفظ می‌شوند و ruleهای قدیمی با ruleهای اصلاح‌شده جایگزین می‌شوند:
...
#### Rapid reconnect fix in 4.2.0

Version 4.1 could feed Paqet's own tunnel packets back into the all-port forwarding path:

- On Abroad, the owner-scoped `REDIRECT` excluded `dport`, although raw Paqet replies carry the tunnel port as their **source** port.
- On Iran, packets arriving with `sport=<tunnel-port>` could enter the TPROXY chain and be handed to Hev.

That formed a recapture loop and produced connections that closed and reopened every few seconds. Version 4.2 adds explicit anti-loop rules:

- Abroad redirects only Paqet target sockets using `! --sport <TUNNEL_PORT>`.
- Iran bypasses all TCP tunnel replies with `--sport <TUNNEL_PORT>` before TPROXY.

It also applies a stable SMUX profile on both hosts: the documented 4MB/2MB buffers, a five-second keepalive, and a 30-second timeout. systemd restart limits prevent an actual crash from turning into an unlimited restart loop. Existing roles, keys, targets, and ports remain preserved on rerun.

The 4.1 improvements—global PREROUTING hook, policy routing, listener verification, and `sudo paqetx diagnose`—remain included.

...
assert_contains "$ABROAD_CONFIG" 'smuxkalive: 5'
assert_contains "$ABROAD_CONFIG" 'smuxktimeout: 30'
assert_contains "$ABROAD_CONFIG" 'smuxbuf: 4194304'
assert_contains "$ABROAD_CONFIG" 'streambuf: 2097152'
...
assert_contains "${ABROAD_ROOT}/usr/local/lib/paqet-x/firewall.sh" '-j REDIRECT'
...
assert_contains "$SELECTED_CONFIG" 'smuxkalive: 5'
assert_contains "$SELECTED_CONFIG" 'smuxktimeout: 30'
...
assert_contains "$ALL_TPROXY_CONFIG" 'mark: 1080'
assert_contains "$ALL_TPROXY_CONFIG" 'connect-timeout: 15000'
assert_contains "$ALL_TPROXY_CONFIG" 'tcp-read-write-timeout: 900000'
assert_contains "$ALL_TPROXY_CONFIG" 'udp-read-write-timeout: 120000'
...
assert_contains "${IRAN_ALL_ROOT}/usr/local/lib/paqet-x/firewall.sh" 'TPROXY --on-ip 127.0.0.1 --on-port'
assert_contains "${IRAN_ALL_ROOT}/usr/local/lib/paqet-x/firewall.sh" 'rp_filter 2'
assert_contains "${IRAN_ALL_ROOT}/usr/local/sbin/paqetx" 'diagnose)'
assert_contains "${IRAN_ALL_ROOT}/usr/local/sbin/paqetx" 'PAQETX_ALLPORT --line-numbers'
assert_contains "${IRAN_ALL_ROOT}/usr/local/sbin/paqetx" 'NRestarts'
assert_contains "${IRAN_ALL_ROOT}/etc/systemd/system/paqet-x.service" 'StartLimitBurst=8'
assert_contains "${IRAN_ALL_ROOT}/etc/systemd/system/paqet-x.service" 'RestartSec=5s'
...
assert_contains "$FW_TEST_LOG" '-A PAQETX_ALLPORT -i lo -j RETURN'
assert_contains "$FW_TEST_LOG" '-A PAQETX_ALLPORT -m mark --mark 1080/0x7ff -j RETURN'
assert_contains "$FW_TEST_LOG" '-A PAQETX_ALLPORT -p tcp --sport 9999 -j RETURN'
assert_contains "$FW_TEST_LOG" '-A PAQETX_ALLPORT -p tcp --dport 1080 -j RETURN'
assert_contains "$FW_TEST_LOG" '-A PAQETX_ALLPORT -p tcp --dport 61080 -j RETURN'
assert_contains "$FW_TEST_LOG" '-t mangle -I PREROUTING -m addrtype --dst-type LOCAL -j PAQETX_ALLPORT'
assert_contains "$FW_TEST_LOG" '-p tcp -j TPROXY --on-ip 127.0.0.1 --on-port 61080 --tproxy-mark 1088/0x7ff'
assert_contains "$FW_TEST_LOG" '-p udp -j TPROXY --on-ip 127.0.0.1 --on-port 61080 --tproxy-mark 1088/0x7ff'
assert_contains "$FW_TEST_LOG" 'ip rule add fwmark 1088/0x7ff table 100'
assert_contains "$FW_TEST_LOG" 'ip route replace local default dev lo table 100'
...
assert_contains "$FW_TEST_LOG" '-m owner --uid-owner 65534 -p tcp \! --sport 9999 \! -d 127.0.0.0/8 -j REDIRECT'
assert_not_contains "$FW_TEST_LOG" '-m owner --uid-owner 65534 -p tcp \! --dport 9999 \! -d 127.0.0.0/8 -j REDIRECT'
assert_contains "$FW_TEST_LOG" '-m owner --uid-owner 65534 -p udp \! -d 127.0.0.0/8 -j REDIRECT'

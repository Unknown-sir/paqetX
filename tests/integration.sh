...
case "$url" in
  ...
  https://github.com/hanselime/paqet/releases/download/*)
    ...
    ;;
  https://api.ipify.org)
    printf '%s' "${MOCK_PUBLIC_IP:-203.0.113.10}"
    ;;
  *)
...
cat > "${MOCK_BIN}/systemd-analyze" <<'MOCK_ANALYZE'
#!/usr/bin/env bash
exit 0
MOCK_ANALYZE

cat > "${MOCK_BIN}/journalctl" <<'MOCK_JOURNAL'
#!/usr/bin/env bash
exit 0
MOCK_JOURNAL
...
ABROAD_TOKEN="${ABROAD_ROOT}/etc/paqet-x/connection.txt"
assert_file "$ABROAD_CONFIG"
assert_file "$ABROAD_TOKEN"
...
assert_contains "$ABROAD_CONFIG" 'block: "aes-128-gcm"'
assert_contains "${ABROAD_ROOT}/etc/paqet-x/firewall.env" 'ROLE=abroad'
assert_contains "${ABROAD_ROOT}/etc/systemd/system/paqet-x.service" 'CAP_NET_BIND_SERVICE'
TOKEN="$(cat "$ABROAD_TOKEN")"
[[ "$TOKEN" == paqetx://* ]] || fail 'abroad token prefix is invalid'
...
assert_contains "$IRAN_CONFIG" 'listen: "0.0.0.0:8443"'
assert_contains "$IRAN_CONFIG" 'addr: "203.0.113.10:9999"'
assert_contains "${IRAN_ROOT}/etc/paqet-x/firewall.env" 'FORWARD_PORTS=443/tcp,8443/tcp'
assert_contains "${IRAN_ROOT}/etc/paqet-x/install.env" 'FORWARD_SPECS=443:443/tcp,8443:8443/tcp'
[[ ! -e "${IRAN_ROOT}/etc/paqet-x/connection.txt" ]] || fail 'Iran role should not retain an abroad connection token file'
...
assert_contains "${IRAN_ROOT}/etc/paqet-x/install.env" 'ROLE=iran'
assert_contains "${IRAN_ROOT}/etc/paqet-x/install.env" 'FORWARD_SPECS=443:443/tcp,8443:8443/tcp'
...
[[ ! -e "${BAD_ROOT}/usr/local/bin/paqet" ]] || fail 'binary was installed after checksum failure'

printf 'Full mocked Abroad/Iran integration tests passed.\n'

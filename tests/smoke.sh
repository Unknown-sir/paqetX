...
grep -Fq 'tcp|udp|both' "${TMP_DIR}/help.out" || fail 'help does not describe protocol selection'
grep -Fq 'Paqet X self-test passed' "${TMP_DIR}/self-test.out" || fail 'self-test success text missing'
grep -Fq 'every TCP/UDP port except the tunnel port' "${TMP_DIR}/help.out" || fail 'help does not describe all-port mode'
grep -Fq 'automatically detected' "${TMP_DIR}/help.out" || fail 'help does not describe automatic SSH detection'
...
expect_failure invalid-forward-mode 'Invalid forward mode' bash "$INSTALLER" --role iran --forward-mode invalid
expect_failure invalid-excluded-port 'Invalid port: bad' bash "$INSTALLER" --role iran --all-ports --exclude-ports bad
expect_failure invalid-ssh-port 'Invalid port: 70000' bash "$INSTALLER" --role iran --all-ports --ssh-ports 70000
expect_failure all-with-selected 'Do not combine --all-ports' bash "$INSTALLER" --role iran --all-ports --ports 443
expect_failure selected-with-exclusions 'valid only with --all-ports' bash "$INSTALLER" --role iran --forward-mode selected --exclude-ports 25
